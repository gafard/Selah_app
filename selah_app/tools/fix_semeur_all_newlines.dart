...
void main() async {
  print('🔧 Réparation de tous les sauts de ligne du fichier semeur.json...');
  
  final inputFile = File('assets/bibles/semeur.json');
  final outputFile = File('assets/bibles/semeur_all_fixed.json');
  ...
  try {
    ...
    // Réparation de tous les sauts de ligne
    String fixedContent = _fixAllNewlines(content);
    
    await outputFile.writeAsString(fixedContent);
    print('✅ Fichier réparé sauvegardé: semeur_all_fixed.json');
...
String _fixAllNewlines(String content) {
  print('🔧 Correction de tous les sauts de ligne...');
  ...
  // 3. NOUVEAU: Corriger TOUS les sauts de ligne littéraux \n dans le JSON
  fixed = _fixAllLiteralNewlines(fixed);
...
String _fixAllLiteralNewlines(String input) {
  // Corriger TOUS les sauts de ligne littéraux \n dans le JSON
  // Pattern: "Guid":"...",\n"Testaments"
