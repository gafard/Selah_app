...
class Criteria {
  ...
  factory Criteria.fromPreset(
    ...
    return Criteria(
      ...
      profile: profile,
      books: [preset.book], // Preset = 1 livre
      order: 'traditional', // Par défaut pour preset
      presetId: preset.id,
...
class PlanService {
  ...
  static Future<Plan> createFromPreset(
    ...
    print('📖 Création plan depuis preset: ${preset.title}');
  ...
  static Future<Plan> _buildPlanWithSharedEngine(Criteria c) async {
    ...
    final adjustedDuration = IntelligentMotivation.adjustDuration(
      baseDuration: durationCalc.optimalDays,
      motivation: c.profile['motivation'],
      heartPosture: c.profile['heartPosture'],
    );
  ...
  static Future<Plan> _assemblePlan({
    ...
    // Convertir EnrichedDay → PlanDay
    final planDays = days.map((e) => PlanDay(
      dayNumber: e.scheduledDay.dayNumber,
      date: e.scheduledDay.date,
      reference: e.scheduledDay.passage.reference,
      book: e.scheduledDay.passage.book,
      startChapter: e.scheduledDay.passage.startChapter,
      endChapter: e.scheduledDay.passage.endChapter,
      estimatedMinutes: e.scheduledDay.passage.estimatedMinutes,
      annotation: e.scheduledDay.passage.annotation,
      hasLiteraryUnit: e.scheduledDay.passage.wasAdjusted,
      unitType: e.scheduledDay.passage.unitType,
      unitPriority: e.scheduledDay.passage.unitPriority,
      tags: e.tags,
    ...
    return Plan(
      id: planId,
      userId: criteria.profile['userId'] ?? 'local',
      title: criteria.presetName ?? _generateCustomTitle(booksPool),
      description: _generateDescription(booksPool, days.length),
      book: booksPool.first.name,
      books: booksPool.map((b) => b.name).toList(),
      duration: days.length,
      minutesPerDay: criteria.minutesPerDay,
      totalMinutes: days.fold(0, (sum, d) => sum + d.scheduledDay.passage.estimatedMinutes),
      ...
      progress: 0,
      isActive: true,
      presetId: criteria.presetId,
      parameters: {
        'durationCalculation': durationCalc.toJson(),
        'booksInfo': booksPool.map((b) => b.toJson()).toList(),
        'criteriaOptions': criteria.options,
      },
    );
  }
